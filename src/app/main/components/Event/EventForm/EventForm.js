...
const EventForm = () => {
  ...
  useEffect(() => {
    if (eventId) {
      axios
        .get(`${eventAPIConfig.getById}/${eventId}`, {
          headers: {
  ...
  useEffect(() => {
    if (eventData) {
      formik.setValues({
        ...
        shivirAvailable:
          eventData.shivirAvailable === true ? "Yes" : "No" || "",
  ...
  const validationSchema = Yup.object().shape({
    ...
    shivirStartDate: Yup.date()
      ...
      .test({
        message:
          "Shivir Start Date should be in between Lock Arrival and Departure Date",
        test: function (value) {
    ...
    shivirEndDate: Yup.date()
      ...
      .test({
        message:
          "Shivir End Date should be in between Lock Arrival and Departure Date",
        test: function (value) {
          const { lockArrivalDate, lockDepartureDate } = this.parent;
          return (
            !lockArrivalDate ||
  ...
  const handleSubmit = (values) => {
    ...
    if (
      ...
      (!values.lockArrivalDate || !values.lockDepartureDate)
    ) {
      dispatch(
        showMessage({
          message: "Lock Dates are required for offline events",
          variant: "error",
        })
      );
      return;
    ...
    if (
      values.eventType === "Offline" &&
      values.shivirAvailable === "Yes" &&
      (!values.shivirStartDate || !values.shivirEndDate)
    ) {
      dispatch(
        showMessage({ message: "Shivir Dates are required", variant: "error" })
      );
      return;
    }
    if (formik.isValid) {
      ...
      formData.append(
        "shivirAvailable",
        values.shivirAvailable === "Yes" ? true : false
      );
      ...
      if (values.file !== null) {
        ...
        axios
          .post(eventAPIConfig.createWithImage, formData, {
            headers: {
              "Content-type": "multipart/form-data",
              authorization: `Bearer ${window.localStorage.getItem(
                "jwt_access_token"
        ...
        axios
          .post(eventAPIConfig.create, formData, {
            headers: {
              "Content-type": "multipart/form-data",
              authorization: `Bearer ${window.localStorage.getItem(
                "jwt_access_token"
            ...
            <form onSubmit={formik.handleSubmit}>
              ...
              <div className={tabValue !== 1 ? "hidden" : ""}>
                ...
                <TextField
                  ...
                  required
                />
                {formik.values.eventType === "Offline" && (
                  <>
                    {" "}
                    <TextField
                      fullWidth
                      ...
                      onBlur={formik.handleBlur}
                      error={
                        formik.touched.lockArrivalDate &&
                        Boolean(formik.errors.lockArrivalDate)
                      }
                      helperText={
                        formik.touched.lockArrivalDate &&
                        formik.errors.lockArrivalDate
                      }
                      sx={{ mb: 2 }}
                      className="max-w-md"
                      required
                    ...
                    <TextField
                      ...
                      onBlur={formik.handleBlur}
                      error={
                        formik.touched.lockDepartureDate &&
                        Boolean(formik.errors.lockDepartureDate)
                      }
                      helperText={
                        formik.touched.lockDepartureDate &&
                        formik.errors.lockDepartureDate
                      }
                      sx={{ mb: 2 }}
                      className="max-w-md"
                      required
              ...
              <div className={tabValue !== 2 ? "hidden" : ""}>
                <Autocomplete
                  ...
                  getOptionLabel={(option) => option}
                  error={
                    formik.touched.shivirAvailable &&
                    Boolean(formik.errors.shivirAvailable)
                  }
                  helperText={
                    formik.touched.shivirAvailable &&
                    formik.errors.shivirAvailable
                  }
                  sx={{ mb: 2 }}
                  className="max-w-md"
                  required
                  renderInput={(params) => (
                    <TextField {...params} label="Shivir Available" />
                  )}
                />
                ...
                {formik.values.shivirAvailable === "Yes" && (
                  <>
                    <TextField
                      ...
                      onBlur={formik.handleBlur}
                      error={
                        formik.touched.shivirStartDate &&
                        Boolean(formik.errors.shivirStartDate)
                      }
                      helperText={
                        formik.touched.shivirStartDate &&
                        formik.errors.shivirStartDate
                      }
                      sx={{ mb: 2 }}
                      className="max-w-md"
                      required
                    ...
                    <TextField
                      ...
                      onBlur={formik.handleBlur}
                      error={
                        formik.touched.shivirEndDate &&
                        Boolean(formik.errors.shivirEndDate)
                      }
                      helperText={
                        formik.touched.shivirEndDate &&
                        formik.errors.shivirEndDate
                      }
                      sx={{ mb: 2 }}
                      className="max-w-md"
                      required
