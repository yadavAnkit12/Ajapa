...
import { clinicAPIConfig, specializationAPIConfig } from '../../API/apiConfig';
...
function FinalCaseHeader(props) {
    ...
    useEffect(() => {
        axios.get(`${clinicAPIConfig.list}`, {
            ...
            if (response.status === 200) {
                ...
                const clinicName = response.data.data.PartnerList.map(item => item.name);
                setClinicOptions(clinicName);
        ...
        <div className="w-full flex flex-col min-h-full">
            ...
            <div className='flex sm:flex-row flex-wrap flex-col justify-between mx-10  mb-10 shadow-1 rounded-16'>
                <div className="flex sm:flex-row flex-wrap flex-col justify-start">
                    ...
                    />}
                      {_.size(clinicOptions) > 0 && <Autocomplete
                        disablePortal
                        value={filterData.clinic}
                        id="caseType"
                        options={clinicOptions}
                        // getOptionLabel={option => option.name}
                        sx={{ my: 1, minWidth: 140, mx: 1 }}
                        onChange={(e, newValue) => setFilterData({ ...filterData, clinic: newValue })}
                        renderInput={(params) => <TextField {...params} label="Clinic" variant="standard" />}
                    />}
                </div>
...
export default FinalCaseHeader;
