...
import { Input, Paper, Typography, Button, Modal, TextField, Autocomplete } from '@mui/material';
import { Box } from '@mui/system';
import { motion } from 'framer-motion';
import { useDispatch, useSelector } from 'react-redux';
import { Link } from 'react-router-dom';
import { useState } from 'react';
import { selectSubscriptionPlanSearchText, setSubscriptionPlanSearchText } from '../../../store/reduxSlice/subscriptionSlice';
import SubscriptionRegisterForm from './SubscriptionRegistrationForm';
import { SUBSCRIPTIONEXPIRYDAY, SUBSCRIPTIONSTATUS } from '../../Static/filterLists';
        ...
        <div className="w-full flex flex-col min-h-full">
                ...
                <div className="flex flex-col w-full sm:w-auto sm:flex-row space-y-16 sm:space-y-0 flex-1 items-center justify-end space-x-8">
                    ...
                    >
                        ...
                        <Input
                            ...
                            onChange={(ev) => dispatch(setSubscriptionPlanSearchText(ev))}
                        />
                        {searchText && <FuseSvgIcon
                            color="disabled"
                            size={16}
                            style={{ cursor: "pointer" }}
                            onClick={() => dispatch(setSubscriptionPlanSearchText(null))
                            }>
                            heroicons-solid:x
                        </FuseSvgIcon>
                        }
            ...
            <div className='flex sm:flex-row flex-wrap flex-col justify-between mx-10  mb-10 shadow-1 rounded-16'>
                <div className="flex sm:flex-row flex-wrap flex-col justify-start">
                    ...
                    />
                    {_.size(SUBSCRIPTIONEXPIRYDAY) > 0 && <Autocomplete
                        disablePortal
                        value={filterData.expiryDay}
                        id="expiryDay"
                        name="expiryDayFilter"
                        options={SUBSCRIPTIONEXPIRYDAY}
                        getOptionLabel={option => option.name}
                        sx={{ my: 1, minWidth: 140, mx: 1 }}
                        onChange={(e, newValue) => setFilterData({ ...filterData, expiryDay: newValue })}
                        renderInput={(params) => <TextField {...params} label="Expiry Day" variant="standard" />}

                    />}
                    {_.size(SUBSCRIPTIONSTATUS) > 0 && <Autocomplete
                        disablePortal
                ...
                <div className="flex flex-row justify-end">
                    <Button
                        component={Link}
                        onClick={filterSubscriptionData}
                        variant="contained"
                        color="secondary"
            ...
            >
                <Box sx={style}>
                    <SubscriptionRegisterForm setOpen={setOpen} open={open} setChange={props.setChange} change={props.change} />
                </Box>
            </Modal>
        </div >
    );
